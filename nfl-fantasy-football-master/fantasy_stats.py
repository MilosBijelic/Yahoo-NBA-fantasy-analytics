...
def UpdatePlayerList_currentSznTotal(): 
        # WEEKLY ROSTERS - TAKES A WHILE
        yahoo_api._login()
        count = [0, 25, 50, 75, 100,125, 150, 175, 200, 225, 250]
		
        for counter in range(1, len(count)): #pulls in batches of 25 players
			#url base for player's current szn total stats
            url = 'https://basketball.fantasysports.yahoo.com/nba/198727/players?status=ALL&pos=P&cut_type=33&stat1=S_S_2020&myteam=0&sort=AR&sdir=1&count=' + counter
            response = oauth.session.get(url, params={'format': 'json'})
            r = response.json()
            file_name = 'playersCurrentSznTotalStats.json'
...
class Bot():
    ...
    def run(self):
        ...
        UD.UpdatePlayerList_currentSznTotal()
        print('Top 300 player list - current season totals created')
